...
function Profile() {
  // Page-level fetch status: 'loading' | 'success' | 'error'
  ...
  const loadAll = useCallback(async () => {
    ...
    setErrorMsg('');
    try {
      const [profileRes, watchlistRes] = await Promise.all([getMyProfile(), getMyWatchlist()]);
      ...
      const message =
        err.response?.status === 401
          ...
          : err.response?.data || 'Could not load your profile. Check your connection and try again.';
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      setProfileMsg(err.response?.data || 'Failed to upload photo.');
    } finally {
    ...
    <div className="profile-page">
      ...
      <div className="profile-sections">
        <form onSubmit={handleProfileSave} className="profile-section">
          ...
          {profileMsg && (
            <p className={profileMsg === 'Saved.' ? 'success' : 'error'}>{String(profileMsg)}</p>
          )}
        ...
        <form onSubmit={handlePasswordChange} className="profile-section">
          ...
          {passwordMsg && (
            <p className={passwordMsg === 'Password updated.' ? 'success' : 'error'}>{String(passwordMsg)}</p>
          )}
